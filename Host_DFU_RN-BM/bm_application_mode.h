...
#ifndef _BM_APPLICATION_MODE_H_
#define _BM_APPLICATION_MODE_H_

#define BM_PKT_LEN_MAX    0x01FF
#define BM_CFG_CMD_SOF_VAL    0xAA

#define BM_APPLICATION_MODE_CMD(x)	((BM_APPLICATION_CMD*)&appCmds[x])

#define BM_APPLICATION_MODE_CMD_VAL(x)	((BM_APPLICATION_CMD_OPCODE)appCmds[x].opcode)
#define BM_APPLICATION_MODE_CMD_LEN(x)	(((BM_APPLICATION_CMD*)&appCmds[x])->length)
...
//#define SERVICE_DISC_CHAR_RESP_ATTRIB_HANDLE_START				(SERVICE_DISC_CHAR_RESP_LENGTH_START + SERVICE_DISC_CHAR_RESP_LENGTH_LEN)
...
typedef enum _bm_application_event_opcode
{
    ...
    BM_SERVER_BLOB_REQUEST = 0xA0,
    BM_TRANSPARENT_DATA_RECEIVED    	= 0x9A,
    BM_SERVER_PREPARE_WRITE_REQ_RECEIVED    	= 0x9E,
    BM_SERVER_EXECUTE_WRITE_REQ_RECEIVED    	= 0x9F,
    BM_ERROR = 0xFF
 ...
 *  \brief Parse a LE end test result event @ref ble_end_test_result_t.
 *  \param data event data.
 *  \param dataLen length of the data.
 *  \pre None.
 *  \return None.
