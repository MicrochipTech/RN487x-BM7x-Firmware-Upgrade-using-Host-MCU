...
//static ble_status_t ble_reset_response_parser(void);
//static ble_status_t ble_adv_data_set_response_parser(void);
//static ble_status_t ble_scan_resp_data_set_response_parser(void);
//static ble_status_t ble_adv_param_set_response_parser(void);
//static ble_status_t ble_adv_start_response_parser(void);
//static ble_status_t ble_set_device_name_response_parser(void);
...
//ble_status_t ble_reset_response_parser(void)
//{
	//bm_cmd_frame_t *cmd_frame = (bm_cmd_frame_t *) get_received_cmd_frame();
	//
	//if(STATUS_REPORT == cmd_frame->event_id)
	//{
		//if(IDLE_MODE == cmd_frame->palyload[0])
		//{
			//return BLE_SUCCESS;
		//}
	//}
...
//}
//
//ble_status_t ble_adv_data_set_response_parser(void)
//{
	//bm_cmd_frame_t *cmd_frame = (bm_cmd_frame_t *) get_received_cmd_frame();
	//
	//if(COMMAND_COMPLETE == cmd_frame->event_id)
	//{
		//if(BM_ADV_DATA_WRITE == cmd_frame->palyload[0])
		//{
			//return BLE_SUCCESS;
		//}
	//}
...
//}
//
//ble_status_t ble_scan_resp_data_set_response_parser(void)
//{
	//bm_cmd_frame_t *cmd_frame = (bm_cmd_frame_t *) get_received_cmd_frame();
	//
	//if(COMMAND_COMPLETE == cmd_frame->event_id)
	//{
		//if(BM_SCAN_DATA_WRITE == cmd_frame->palyload[0])
		//{
			//return BLE_SUCCESS;
		//}
	//}
...
//}
//
//ble_status_t ble_adv_param_set_response_parser(void)
//{
	//bm_cmd_frame_t *cmd_frame = (bm_cmd_frame_t *) get_received_cmd_frame();
	//
	//if(COMMAND_COMPLETE == cmd_frame->event_id)
	//{
		//if(BM_ADV_PARAM_SET == cmd_frame->palyload[0])
		//{
			//return BLE_SUCCESS;
		//}
	//}
...
//}
//
//ble_status_t ble_adv_start_response_parser(void)
//{
	//bm_cmd_frame_t *cmd_frame = (bm_cmd_frame_t *) get_received_cmd_frame();
	//
	//if(COMMAND_COMPLETE == cmd_frame->event_id)
	//{
		//if(BM_SET_ADV_ENABLE == cmd_frame->palyload[0])
		//{
			//return BLE_SUCCESS;
		//}
	//}
...
//}
//
//ble_status_t ble_set_device_name_response_parser(void)
//{
	//bm_cmd_frame_t *cmd_frame = (bm_cmd_frame_t *) get_received_cmd_frame();
	//
	//if(COMMAND_COMPLETE == cmd_frame->event_id)
	//{
		//if(BM_NAME_WRITE == cmd_frame->palyload[0])
		//{
			//return BLE_SUCCESS;
		//}
	//}
...
{
	ble_status_t status = BLE_INVALID_PARAM;
...
ble_status_t ble_pair_request(ble_handle_t conn_handle)
{
	...
	ble_uart_send_sync(application_cmdpkt->cmdPkt, (uint16_t) application_cmdpkt->length);
...
ble_status_t ble_remote_device_name_read(uint8_t conn_handle, uint8_t *name, uint8_t *name_length)
{
	...
	//{
		///* Check the status in status-report event */
		//status = ble_remote_device_name_read_parser(name, name_length);
	//}
...
ble_status_t ble_primary_service_discover_all(ble_handle_t conn_handle)
{
	...
	{
		/* Check the status in status-report event */
		status = ble_command_complete_parser(BM_CLIENT_DISCOVER_ALL_SERVICES);
	}
...
ble_status_t ble_write_response_send(ble_handle_t conn_handle, ble_handle_t attr_handle, uint8_t err_code)
{
	...
	BM_APPLICATION_ServerWriteResponseSendPkt(application_cmdpkt, conn_handle, 0x1, attr_handle, err_code);
	status = interface_cmd_send_wait(application_cmdpkt->cmdPkt, (uint32_t) application_cmdpkt->length, COMMAND_COMPLETE);
	if(BLE_SUCCESS == status)
	{
		/* Check the cmd_id in command complete event */
		status = ble_command_complete_parser(0x01);
	}
...
ble_status_t ble_characteristic_reliable_write_response_prepare(ble_handle_t conn_handle, ble_handle_t characteristic_handle, uint16_t offset, uint8_t* data, uint16_t length)
{
	...
	{
		/* Check the cmd_id in command complete event */
		status = ble_command_complete_parser(0x02);
	}
...
ble_status_t ble_characteristic_reliable_write_response_execute(ble_handle_t conn_handle)
{
	...
	{
		/* Check the cmd_id in command complete event */
		status = ble_command_complete_parser(0x03);
	}
...
ble_status_t ble_gatt_service_create(ble_service_t *ble_service)
{
	...
	for(char_index = 0; char_index < ble_service->char_count; char_index++)
	{
		...
		{
			return BLE_ATT_INSUFF_RESOURCE;
		}
		...
		for(desc_index = 0; desc_index < ble_service->char_list[char_index].additional_desc_count; desc_index++)
		{
			...
			else if(BLE_UUID_128B == ble_service->char_list[char_index].additional_desc_list[desc_index].desc_uuid.type)
			{
				...
				//gen_att_private->desc_val_max_length = BLE_ATT_ATTRIBUTE_VALUE_LEN;
