...
//BM_PROGRAM_EVENT                    pgmEvent = {BM_PROGRAM_HCI_ERROR, NULL, 0};
...
{  
    ...
    crc_pkt->acl_data_pkt.conn_handle = pgm_cmd->handle;//0x0FFF;    //fill
...
/*! \fn BM_PROGRAM_EVENT* BM_PROGRAM_ResponseCheck(uint8_t* data, uint8_t len)
 ...
 *  \pre Program mode connect function mut be called.
 *  \return Returns the handle to the event.
 */
        ...
        switch(pgmCmdState)
        {
            case BM_PGM_HCI_EVENT:
                if(*data == BM_PGM_HCI_EVENT_VAL || *data == BM_PGM_HCI_ACL_VAL)
                {
                    ...
                    //pgmEvent.eventID = BM_PROGRAM_HCI_ERROR;
                    //pgmEvent.data = NULL;
                    //pgmEvent.length = 0;
					appEvent.event_id = BM_PROGRAM_HCI_ERROR;
					appEvent.event_msg.data = NULL;
            ...
            case BM_PGM_HCI_EVENT_DATA:
                ...
                if(pgmDataIndex >= pgmRspData[BM_PGM_HCI_EVENT_LEN_INDEX])
                {
                    ...
                    {
                        //pgmEvent.eventID = pgmRspData[BM_PGM_HCI_EVENT_INDEX];
                        //pgmEvent.data = &pgmRspData[BM_PGM_HCI_EVENT_DATA_INDEX];
                        //pgmEvent.length = pgmRspData[BM_PGM_HCI_EVENT_LEN_INDEX];
            ...
            case BM_PGM_HCI_ACL_DATA:
                {
                    ...
                    if(pgmDataIndex >= (pgmRspData[BM_PGM_HCI_ACL_LENH_INDEX]<<8 | pgmRspData[BM_PGM_HCI_ACL_LENL_INDEX]))
                    {
                        ...
                        //pgmEvent.eventID = pgmRspData[BM_PGM_HCI_EVENT_INDEX];
                        //pgmEvent.data = &pgmRspData[BM_PGM_HCI_ACL_DATA_INDEX];
                        //pgmEvent.length = (pgmRspData[BM_PGM_HCI_ACL_LENH_INDEX]<<8 | pgmRspData[BM_PGM_HCI_ACL_LENL_INDEX]);
